...
class InterfazUsuario:
    ...
    def agregar_habito(self):
        ...
        try:
            ...
            # Solicitar nombre
            nombre = input("🏷️ Nombre del hábito: ").strip()
            if not nombre:
            ...
            print("1. Diaria")
            print("2. Semanal")
            opcion_freq = input("Selecciona (1-2): ").strip()
            ...
            horario = self._solicitar_horario()
            if horario is False:  # Error en el formato
                self.pausar()
                return
    ...
    def marcar_habito_completado(self):
        ...
        try:
            opcion = int(input(f"\n🎯 Selecciona el hábito a completar (1-{len(habitos_pendientes)}): "))
            
            if 1 <= opcion <= len(habitos_pendientes):
                habito_seleccionado = habitos_pendientes[opcion - 1]
    ...
    def mostrar_progreso_habitos(self):
        ...
        try:
            opcion = int(input(f"\n🎯 Selecciona una opción (1-{len(habitos) + 1}): "))
            
            if 1 <= opcion <= len(habitos):
                self.mostrar_progreso_detallado(habitos[opcion - 1].id)
            elif opcion == len(habitos) + 1:
                return
            else:
                print("⚠️ Selección no válida.")
                self.pausar()
    ...
    def gestionar_habitos(self):
        ...
        while True:
            ...
            try:
                opcion = int(input(f"\n🎯 Selecciona un hábito para gestionar (1-{len(habitos) + 1}): "))
                
                if 1 <= opcion <= len(habitos):
                    self.gestionar_habito_individual(habitos[opcion - 1])
                elif opcion == len(habitos) + 1:
                    return
                else:
                    print("⚠️ Selección no válida.")
    ...
    def mostrar_historial(self):
        ...
        try:
            ...
            if 1 <= opcion <= len(habitos):
                self.mostrar_historial_habito(habitos[opcion - 1])
            elif opcion == len(habitos) + 1:
                return
            else:
                print("⚠️ Selección no válida.")
    ...
    def mostrar_historial_habito(self, habito):
        ...
        try:
            ...
            if opcion == '1':
                ...
                if dias <= 0:
                    print("⚠️ Número de días inválido.")
                    self.pausar()
                    return
            else:
                print("⚠️ Opción no válida.")
